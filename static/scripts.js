...
// Function to pass the signature to the front ID card
function passSignatureToFront() {
    const canvas = document.getElementById('canvas'); // Get the canvas element
    const signaturePreviewFront = document.getElementById('signature-preview-front'); // Get the signature placeholder

    // Convert canvas to data URL (PNG format)
    const dataURL = canvas.toDataURL('image/png');

    // Set the signature preview image source
    signaturePreviewFront.src = dataURL;
    signaturePreviewFront.style.display = 'block'; // Make the image visible

    closeModal(); // Close the signature modal
}
...
// Function to print the back ID card
function printBack() {
    html2canvas(document.getElementById('back-id-card')).then(canvas => {
        const printWindow = window.open('');
        printWindow.document.write('<img src="' + canvas.toDataURL('image/png') + '">');
...
// Event listener for thickness control and resizable
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    interact('.resizable-signature')
        .draggable({
            inertia: true,
            modifiers: [
                interact.modifiers.restrictRect({
                    restriction: signatureBox, // Restrict movement within the signature box
                    endOnly: true
                })
            ],
